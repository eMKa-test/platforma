...
function uploadAsChunk(file, uuid,
    ...
    const readEventHandler = function countChunks(evt) {
        ...
        if (evt.target.error == null) {
            ...
            fetchRequest("post", "/agent/api/content/tv/", param)
                .then(({ data }) => {
                    if (data.success) {
                        if (offset >= fileSize) {
...
class UploadItemsStream extends Component {
    state={
        progress: 0,
        total: 0,
        content: [],
    ...
    chunkSubmitUploadRequest = () => {
        ...
        const uuid = `STREAM_${moment().format("x")}`;
        if (content[progress]) {
    ...
    callNextItem = () => {
        ...
        this.setState(
            ...
            () => {
                ...
                this.props.fetchUploadedStream();
            },
        );
    }
    ...
    async submitUploadRequest() {
        ...
        for (let file of this.state.content) {
            ...
            try { // eslint-disable-next-line
                await fetchRequest("post", "/agent/api/content/tv/", param, this.setConfig(total))
                    .then((res) => {
                        if (res.data.success) {
                            this.props.changeBGHeader("default");
                            this.setState(
                ...
                if (e.response && e.response.data.error === "authenticationFailed") {
                    alert("Вы не авторизованы.");
                    window.location.href = "/agent/login";
                    break;
                }
    ...
    setNewStateProgress = () => this.setState({
        progress: 0, total: 0, currentLoadDone: false, errorsContent: [], fatProgress: 0, content: [], attempt: 0,
    });
            ...
            <Card>
                <CardBody>
                    ...
                    <div
                        ...
                        <LoadInfo
                            handleFunc={clearItemContentStream}
                            {...this.props}
                            {...this.state} />
                    </div>
                </CardBody>
